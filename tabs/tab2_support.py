...
def render() -> None:
    ...
    gen_options = st.multiselect(
        "生成したい項目を選択",
        ["スカウトメール", "トークスクリプト", "懸念点への切り返しトーク", "キャリアパス", "フォローメール", "企業への推薦文"],
...
（まとめ）"""
        ...
        # ── フォローメール ──
        if "フォローメール" in gen_options:
            st.subheader("面談後フォローメール")
            with st.spinner("フォローメールを生成中..."):
                follow_prompt = f"""あなたは設備管理・施工管理業界に特化した人材紹介エージェントです。
候補者と面談した後に送る、フォローメールを作成してください。
...
面談後のフォローメールとして、以下の2パターンを作成:

## パターン1: 候補者が前向きだった場合
**件名:**
**本文:** （面談のお礼→企業の魅力再確認→次のステップの明確化→期限付きアクション誘導）

## パターン2: 候補者が迷っている場合
**件名:**
**本文:** （面談のお礼→候補者の懸念への共感→新たな角度からの情報提供→プレッシャーをかけずに次の接点を作る）

※メールとしてそのまま送れる完成度で作成してください"""
                follow_result = call_claude(follow_prompt)
            st.markdown(follow_result)
            with st.expander("コピー用テキスト"):
                st.code(follow_result, language=None)

        # ── 企業への推薦文 ──
        if "企業への推薦文" in gen_options:
            st.subheader("企業への候補者推薦文")
            with st.spinner("推薦文を生成中..."):
                rec_prompt = f"""あなたは設備管理・施工管理業界に特化した人材紹介エージェントです。
以下の候補者を企業に推薦する推薦文を作成してください。
...
**候補者の志望動機:** （候補者がなぜこの企業に興味を持っているか）

**補足事項:** （年収・入社時期等の条件面で企業に伝えておくべきこと）

※企業の採用担当者が「この人に会いたい」と思える内容にしてください"""
